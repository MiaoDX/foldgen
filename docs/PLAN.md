...
- **后续 stage**：Stage 1 的当前硬门槛是 repo-local technical gate。人手、机器手、工具夹具或其他形态的 physical executor 复现只作为最终发布 claim gate，不阻塞当前技术迭代。
...
- M4: Stage 1 technical closeout：README、demo 和 pipeline 均标注为 simulator-valid / embodiment-untested；博客和发布材料只准备草稿，不要求外部 executor 参与。

## 成功标准

- **绝对**：展示 5 个 simulator-valid demo case，输出 FOLD、crease pattern、preview 和 executor-readable diagram，且默认不需要外部人参与。
